...
void read_line(FILE* file, char* line) //Reads from a file line by line. Every time you call this function, you move through the file.
{
    char ch = getc(file);
    int count = 0;
...
void generate_field(int size, struct position* positions) //Simply generates the field by it's date for pieces.
{
    char letter = 65;
    int number = size;
    int count = 0;
...
char is_new_position(struct position* positions, int count, int x, int y) //Checks if there is a duplication in the coordinates of the pieces.
{
...
void generate_positions(int size, struct position* positions) //Creates random coordinates for the pieces. There is a specific way to do this. More info in the README.
{
    time_t t;
...
void is_enemy_check_or_checkmate(int size, struct position* positions, int* result, int* checks) //Checks if the game is about to end. (Check or checkmate)
{
    int flag = 0;
    int save_pos[2];
    ...
    do { //KE is moving by this "do-while" loop.
    ...
    } while((positions[0].x == save_pos[0] && positions[0].y == save_pos[1]) || (positions[0].x>save_pos[0]+1 || positions[0].x<save_pos[0]-1) || (positions[0].y>save_pos[1]+1 || positions[0].y<save_pos[1]-1) ||
...
void change_position(int size, struct position* positions) //This is the MAIN function of the program. It allows you to move pieces and informs you if you have invalid moves.
{
    char piece[3] = {"NO"};
    char pos[4];
    ...
    while(is_valid_position == 0){ //The BIG check for invalid moves.
...
void replay_change_position(int size, struct position* positions, char* current_piece, char* current_pos) //The same function as the previous one. It is made for replays. The BIG check is not removed for debugging.
{
    char piece[3] = {"NO"};
    char pos[4];
    int is_valid_position = 0;
...
int main()
{
    ...
    int choice;
    int checks;
    struct position positions[4] = {{"KE", 0, 0, 0}, {"KF", 0, 0, 0}, {"R1", 0, 0, 0}, {"R2", 0, 0, 0}}; //NOTE: KE = King Enemyl KF = King Friendly; R1 & R2 = Rook 1 & 2
    ...
    while(1) { //The main menu.
        ...
        switch(choice)
        {
            case 1: { //The whole game. Uses every function.
                FILE *replay;
            ...
            case 2: //Simply changes the size.
                printf("Choose size from 6, 8 or 10: ");
                scanf("%d", &size);
                while(size != 6 && size != 8 && size != 10) {
            ...
            case 3: { //The replay part.
                ...
                int size = atoi(line);
                for(int i = 0; i < 8; i++) { //Saves the coordinates from the very begging.
                    read_line(replay, &line);
                    array[i] = atoi(line);
                }
                ...
                while(strcmp(line, "END") != 0) {
                    read_line(replay, &line);
                    switch(counter) //Saves the moves, made by us at that time (replay).
                    {
    ...
    return 0; //The END (caused by errors).
}
